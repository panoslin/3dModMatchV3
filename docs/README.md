...
## 📒 TODO
- 现在完成了基本业务逻辑
  - 一个问题，我匹配了一个粗胚分类，只显示了一个匹配结果？
- 优化
  - **可视化应该一比一还原**
  - 清除废物
  - 构建 windows 版本
  - ~~页面优化~~
    - ~~粗胚管理，更加 compact 的设计，使用树状结构显示~~
  - ~~tab 1： 粗胚预览界面，应该显示基本信息，比如 volume，名字等~~
  - tab 2：优化界面布局，更专业、production-ready
  - ~~tab 2: p96 间隙值一直为 0 -- 检查为什么为 0，是算法没有返回还是前端没有正确接收~~
  - ~~tab 2：匹配完成后，计时器还在计时~~
  - ~~detailed 视图，应该显示所有粗胚的匹配结果，而不是只显示最优匹配结果~~
  - detailed 视图，加载比较慢，找出瓶颈，并且找出有什么加速方式？
  - tab 2: 分类信息不是事实更新现实，需要每次打开 tab 2 都更新加载一遍 分类信息
  - tab 2：显示结果有问题
  	- 
